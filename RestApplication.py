...
con=sqlite3.connect('KLIENT_SERW.db')
...
with open('restaurants.json', 'r',encoding='utf-8') as json_file:
    restaurants=json.load(json_file)

with open('listOfRestaurants3.json', 'r',encoding='utf-8') as json_file:
...
def Clicked():
    ...
    if restName in listWidget.item(listWidget.currentRow()).text():
        ...
        distance=sqrt((float(latRest)-float(latUser))**2+(cos((float(latUser)*pi)/180)*(float(lngRest)-float(lngUser)))**2)*(40075.704/360)
...
def showRest():
    ...
    for rest in restDatabase:
        listWidget.addItem("Nazwa: " + rest['NAZWA'] + "\n" + "Ocena: " + str(rest['Ocena'])  + "\n" + "Ilość wolnych stolików: " + str(rest['IloscStol']))
    ...
    for i in range(len(listOfRandnumber)):
        for rest in restaurantsYelp:
            if listOfRandnumber[i] == rest["Number"]:
                ...
                cur.execute('INSERT INTO REST VALUES(?, ?, ?, ?, ?, ?);', (rest["Name"], str(rest["Rating"]), str(rest["Latitude"]), str(rest["Longitude"]), str(price)),"Kebab")
    con.commit()
        ...
        if content == "Kebab":
              for rest in restaurants:
                if content in rest["kitchens"]:
                    ...
                    #cur.execute('INSERT INTO KITCHENS VALUES(?, ?);', (content, str(rest["menu"])))
